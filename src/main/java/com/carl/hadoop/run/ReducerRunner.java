...
public class ReducerRunner extends AbstractRunner {
    ...
    public final boolean runHadoop() {
        ...
        if (StringUtils.isNotBlank(this.contextConfig.getRunnerParam().getInitMethod())) {
        ...
        //调用执行该类父类的回调方法
        if (StringUtils.isNotBlank(this.contextConfig.getRunnerParam().getCallback())) {
    ...
    private void invokeReduce(String path) {
        ...
        BufferedReader reader = com.carl.hadoop.utils.IOUtils.createReader(path);
